...
:::note[Поток данных]
Вход: демо-база (dvdrental / orders) залита в PostgreSQL
→ Обработка: пишешь SQL в DBeaver, сервер выполняет запрос
→ Выход: таблица результата в окне DBeaver.
Зачем: локальная песочница, где безопасно тренировать SQL на реальных таблицах.
:::

Как эти две программы связаны: сервер хранит и считает, клиент — твоё окно к нему.

```mermaid
flowchart LR
    DB["PostgreSQL-сервер<br/>хранит данные, порт 5432"] <-->|подключение| GUI["DBeaver<br/>GUI-клиент"]
    GUI -->|SQL-запрос| DB
    DB -->|результат| GUI
    GUI --> YOU["Ты: пишешь запрос<br/>и видишь таблицу"]
```
