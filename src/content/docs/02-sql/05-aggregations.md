...
:::note[Поток данных]
Вход: много строк (заказы)
→ Обработка: `GROUP BY` бьёт на группы → агрегат (`SUM`/`COUNT`/`AVG`) сворачивает каждую → `HAVING` отсекает группы
→ Выход: таблица «метрика по группам».
Зачем: превратить сырые строки в сводные цифры (выручка по странам, средний чек).
:::
