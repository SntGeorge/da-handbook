...
:::note[Поток данных]
Вход: таблица(ы)
→ Обработка: `WITH` даёт промежуточному результату имя → следующие шаги ссылаются на него (цепочкой или рекурсивно)
→ Выход: итоговая таблица.
Зачем: разбить сложный запрос на читаемые именованные шаги вместо вложенных подзапросов.
:::
