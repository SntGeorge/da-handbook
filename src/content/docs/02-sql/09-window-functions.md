...
:::note[Поток данных]
Вход: строки таблицы
→ Обработка: функция считается **по окну** (`PARTITION BY` + `ORDER BY`), но строки НЕ схлопываются
→ Выход: те же строки + новый столбец (ранг, нарастающий итог, доля, значение соседа).
Зачем: добавить агрегат/ранг рядом с каждой строкой — то, что `GROUP BY` не умеет.
:::
