...
:::note[Поток данных]
Вход: столбцы с датами и текстом
→ Обработка: функции дат (`DATE_TRUNC`, `EXTRACT`, интервалы) и строк (`SUBSTRING`, `LOWER`, `LIKE`) приводят/режут значения
→ Выход: нужные срезы по времени и очищенный/разобранный текст.
Зачем: «по месяцам», «за последние 30 дней», нормализация и парсинг строк.
:::
