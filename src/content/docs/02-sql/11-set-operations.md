...
:::note[Поток данных]
Вход: два результата `SELECT` с одинаковыми столбцами
→ Обработка: `UNION` (объединить, убрав дубли), `UNION ALL` (со всеми), `INTERSECT` (общее), `EXCEPT` (разница)
→ Выход: один объединённый набор строк.
Зачем: сложить выгрузки/периоды в одну таблицу или сравнить два набора.
:::
