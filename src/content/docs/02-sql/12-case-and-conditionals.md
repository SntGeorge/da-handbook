...
:::note[Поток данных]
Вход: строки таблицы
→ Обработка: `CASE WHEN ...` присваивает значение по условию; `COALESCE`/`NULLIF` обрабатывают `NULL`
→ Выход: новый производный столбец (категория, флаг, заглушка вместо NULL).
Зачем: построить логику «если-то» прямо в запросе — сегменты, бакеты, защита от NULL.
:::
