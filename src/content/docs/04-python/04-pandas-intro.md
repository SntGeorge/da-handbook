...
:::note[Поток данных]
Вход: источник (CSV / Excel / JSON / БД)
→ Обработка: `read_*` загружает в DataFrame, осмотр через `head`/`info`/`describe`
→ Выход: таблица в памяти, готовая к фильтрам, группировкам и графикам.
Зачем: превратить файл/выгрузку в объект, с которым удобно работать кодом.
:::
