...
:::note[Поток данных]
Вход: подготовленный DataFrame (после фильтров/группировок)
→ Обработка: функция графика (`plot`/`bar`/`hist`/Seaborn) рисует по столбцам
→ Выход: статичная картинка (PNG в отчёт/ноутбук).
Зачем: увидеть тренд, выброс, распределение глазами — быстрее, чем по таблице.
:::
