...
:::note[Поток данных]
Вход: файл (CSV / Excel / JSON / Parquet)
→ Обработка: `read_*` с правильными параметрами (encoding, sep, decimal); `to_*` для записи
→ Выход: DataFrame в памяти (или сохранённый файл результата).
Зачем: корректно прочитать «сырой» файл — первый шаг любого анализа.
:::
