...
:::note[Поток данных]
Вход: внешний источник в вебе (API или HTML-страница)
→ Обработка: `requests` забирает ответ → парсинг (`.json()` или BeautifulSoup) → `json_normalize` в таблицу
→ Выход: DataFrame с данными из веба.
Зачем: получить данные, которых нет в твоей БД (курсы, погода, вакансии, реестры).
:::
