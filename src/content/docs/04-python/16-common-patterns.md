...
Если `p-value < 0.05` — различие статистически значимо. Для **долей-конверсий** канонически берут z-тест двух пропорций (`statsmodels.proportions_ztest`) или `chi2_contingency`; `ttest_ind` на значениях 0/1 тоже работает как приближение. Полная методология (размер выборки, ошибки, подводные камни) — в [разделе A/B](/09-ab-testing/01-fundamentals/).
