...
В СНГ ClickHouse — почти стандарт для продуктовой аналитики (его используют Яндекс и множество компаний вокруг). Если ищешь работу здесь, велик шанс работать именно с ним, а не со Snowflake/BigQuery ([обзор стека](/00-intro/market-stack-2026/)). Он невероятно быстр на агрегатах за счёт колоночного хранения, сжатия и векторизованного выполнения — типичный сценарий — аналитика событий: миллиарды строк логов/кликов, по которым нужны быстрые `GROUP BY`.
