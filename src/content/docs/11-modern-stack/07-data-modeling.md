...
:::caution[Первый шаг Кимбалла — объявить гранулярность (grain)]
До проектирования таблицы фактов чётко сформулируй, **что есть одна строка**: «одна строка = одна позиция в заказе» или «один заказ» или «один платёж». Это и есть **grain**. Все метрики и ключи должны соответствовать этому уровню. Самая частая ошибка — смешать гранулярности (строки заказа и сам заказ в одной таблице) → задвоение сумм при агрегации. Сформулировал грейн одним предложением — дальше всё проектируется однозначно.
:::
...
Как выглядит измерение SCD Type 2 — у клиента две версии-строки с интервалами действия и флагом текущей:

| customer_sk | customer_id | city | valid_from | valid_to | is_current |
|-------------|-------------|------|------------|----------|------------|
| 101 | C-1 | Москва | 2024-01-01 | 2026-03-15 | false |
| 102 | C-1 | Берлин | 2026-03-15 | 9999-12-31 | true |

Факт ссылается на `customer_sk` (суррогатный ключ конкретной версии), поэтому старый заказ навсегда привязан к «Москве», а новый — к «Берлину». Отчёт «как сейчас» фильтрует `is_current = true`.
